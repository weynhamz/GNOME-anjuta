...
{
	FileManagerPlugin *fv = FILE_MANAGER_PLUGIN (user_data);
					...
					const GValue *value, gpointer user_data)
{
	...
	fm_plugin = FILE_MANAGER_PLUGIN (plugin);
	...
	if (root_uri)
	{
		...
		if (root_dir)
		{
			...
			FILE_MANAGER_PLUGIN (plugin)->project_is_loaded = TRUE;
		}
		else
...
project_root_removed (AnjutaPlugin *plugin, const gchar *name,
					  gpointer user_data)
{
	set_default_root_directory (FILE_MANAGER_PLUGIN (plugin));
	FILE_MANAGER_PLUGIN (plugin)->project_is_loaded = FALSE;
}
...
{
	FileManagerPlugin *plugin = FILE_MANAGER_PLUGIN (obj);
	plugin->tree = NULL;
...
{
	fv_set_root (FILE_MANAGER_PLUGIN (file_manager), root);
}
...
{
	return fv_get_selected_file_path(FILE_MANAGER_PLUGIN (file_manager));
}
...
{
	FileManagerPlugin* fm_plugin = FILE_MANAGER_PLUGIN (ipref);
	/* Add preferences */
