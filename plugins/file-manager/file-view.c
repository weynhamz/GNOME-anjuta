...
file_view_expand_idle (gpointer data)
{
	...
	if (expand->files)
	{
		...
		gchar* uri = g_build_filename (expand->uri,
											info->name, NULL);
...
static int
file_view_sort (gconstpointer a,
				gconstpointer b)
{
...
static void
file_view_row_expanded(GtkTreeView* tree_view,
						GtkTreeIter* iter,
					    GtkTreePath* path)
{
	AnjutaFileView* view = 
	...
	if (gnome_vfs_directory_list_load(&files, 
								 uri,
								 GNOME_VFS_FILE_INFO_GET_MIME_TYPE) ==
		GNOME_VFS_OK)
	{
		files = g_list_sort (files, file_view_sort);
		FileViewIdleExpand* expand =
			g_new0(FileViewIdleExpand, 1);
		expand->files = files;
		expand->path = gtk_tree_model_get_path (priv->model,
												iter);
		expand->view = view;
		expand->uri = uri;
...
static void
file_view_row_collapsed(GtkTreeView* tree_view,
						GtkTreeIter* iter,
						GtkTreePath* path)
{
	AnjutaFileView* view = 
	...
	while (gtk_tree_model_iter_children(priv->model,
										&child,
										iter))
	{
...
static void
file_view_on_file_clicked(GtkTreeViewColumn* column, 
						  AnjutaFileView* view)
{
...
static void
file_view_save_expanded_row (GtkTreeView* tree_view,
							 GtkTreePath* path,
							 gpointer user_data)
{
	AnjutaFileView* view = 
...
file_view_get_selected (AnjutaFileView* view)
{
	...
	gchar* uri;
	if (gtk_tree_selection_get_selected (selection, priv->model, 
									 &selected))
	{
...
static gboolean
file_view_button_press_event (GtkWidget* widget,
							  GdkEventButton* event)
{
	...
	{
		GtkTreePath* path =
							gtk_tree_model_get_path(priv->model,
													&selected);
		gtk_tree_model_get (priv->model, &selected,
							COLUMN_URI, &uri,
...
static void
file_view_selection_changed (GtkTreeSelection* selection,
							 AnjutaFileView* view)
{
	GtkTreeIter selected;
	GtkTreeModel* model;
...
static void
file_view_get_property (GObject *object,
			guint prop_id,
			GValue *value,
			GParamSpec *pspec)
{
...
static void
file_view_set_property (GObject *object,
			guint prop_id,
			const GValue *value,
			GParamSpec *pspec)
{
...
{
	return g_object_new(ANJUTA_TYPE_FILE_VIEW, "base_uri",
						NULL, NULL);
}
