...
sample_plugin_activate_plugin (AnjutaPlugin *plugin)
{
	...
	sample_plugin = ANJUTA_SAMPLE_PLUGIN (plugin);
...
sample_plugin_deactivate_plugin (AnjutaPlugin *plugin)
{
	...
	anjuta_shell_remove_widget (plugin->shell, ANJUTA_SAMPLE_PLUGIN (plugin)->widget,
								NULL);
	anjuta_ui_unmerge (ui, ANJUTA_SAMPLE_PLUGIN (plugin)->uiid);
	return TRUE;
}
...
{
	SamplePlugin *plugin = ANJUTA_SAMPLE_PLUGIN (obj);
	plugin->uiid = 0;
}
