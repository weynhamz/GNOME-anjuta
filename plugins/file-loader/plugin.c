...
on_create_submenu (gpointer user_data)
{
	...
	loader = ANJUTA_FILE_LOADER_PLUGIN (user_data);
							...
							const GValue *value, gpointer data)
{
	...
	fl_plugin = ANJUTA_FILE_LOADER_PLUGIN (plugin);
...
activate_plugin (AnjutaPlugin *plugin)
{
	...
	loader_plugin = ANJUTA_FILE_LOADER_PLUGIN (plugin);
...
dispose (GObject *obj)
{
	AnjutaFileLoaderPlugin *plugin = ANJUTA_FILE_LOADER_PLUGIN (obj);
			  ...
			  gboolean read_only, GError **err)
{
	...
	if (mime_type == NULL)
	{
		launch_application_failure (ANJUTA_FILE_LOADER_PLUGIN (loader),
	...
	{
		launch_in_default_application (ANJUTA_FILE_LOADER_PLUGIN (loader), mime_type, uri);
	}
	if (plugin)
		ianjuta_file_open (IANJUTA_FILE(plugin), uri, NULL);
	
	set_recent_file (ANJUTA_FILE_LOADER_PLUGIN (loader), new_uri, mime_type);
