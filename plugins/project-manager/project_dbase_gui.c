...
create_project_menus (ProjectDBase * p)
{
	...
	gtk_widget_hide(menu1_uiinfo[8].widget); /* Help unimplemented */
	gtk_widget_ref (menu);
}
...
create_project_dbase_gui (ProjectDBase * p)
{
	...
	anjuta_shell_add_widget (ANJUTA_SHELL (app), fv->win,
							"AnjutaFileBrowser", "Files", NULL);

	sv = sv_populate (build_sv);
	anjuta_shell_add_widget (ANJUTA_SHELL (app), sv->win,
							 "AnjutaSymbolBrowser", "Symbols", NULL);
	scrolledwindow1 = gtk_scrolled_window_new (NULL, NULL);
	anjuta_shell_add_widget (ANJUTA_SHELL (app), scrolledwindow1,
							 "AnjutaProjectManager", "Project", NULL);
...
create_project_dbase_info_gui (gchar * lab[])
{
	...
	gtk_window_set_transient_for (GTK_WINDOW(dialog1), GTK_WINDOW(app));
