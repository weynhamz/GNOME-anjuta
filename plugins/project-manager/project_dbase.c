...
project_dbase_clear (ProjectDBase * p)
{
	...
	p->project_is_open = FALSE;
	p->clean_before_build = FALSE;
	gtk_widget_set_sensitive(app->menubar.file.recent_projects, TRUE);
	p->is_saved = TRUE;
...
project_dbase_close_project (ProjectDBase * p)
{
	...
	if (p->is_saved == FALSE)
	{
		...
		dialog = create_project_confirm_dlg (GTK_WIDGET (app));
...
project_dbase_update_controls (ProjectDBase * pd)
{
	...
	gtk_widget_set_sensitive (app->menubar.cvs.update_project, p);
	gtk_widget_set_sensitive (app->menubar.cvs.commit_project, p);
	gtk_widget_set_sensitive (app->menubar.cvs.import_project, p);
	gtk_widget_set_sensitive (app->menubar.cvs.status_project, p);
	gtk_widget_set_sensitive (app->menubar.cvs.log_project, p);
	gtk_widget_set_sensitive (app->menubar.cvs.diff_project, p);
}
...
project_dbase_load_project_file (ProjectDBase * p, gchar * filename)
{
	...
	gtk_widget_set_sensitive(app->menubar.file.recent_projects, FALSE);
