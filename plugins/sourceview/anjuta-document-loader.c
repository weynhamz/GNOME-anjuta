...
#define ANJUTA_DOCUMENT_LOADER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						  ANJUTA_TYPE_DOCUMENT_LOADER,            \
						  AnjutaDocumentLoaderPrivate))
...
struct _AnjutaDocumentLoaderPrivate
{
	...
	const AnjutaEncoding      *auto_detected_encoding;

	GError                   *error;
};

G_DEFINE_TYPE(AnjutaDocumentLoader, anjuta_document_loader, G_TYPE_OBJECT)
...
static void
anjuta_document_loader_set_property (GObject      *object,
				    guint         prop_id,
				    const GValue *value,
				    GParamSpec   *pspec)
{
	AnjutaDocumentLoader *dl = ANJUTA_DOCUMENT_LOADER (object);
...
static void
anjuta_document_loader_get_property (GObject    *object,
				    guint       prop_id,
				    GValue     *value,
				    GParamSpec *pspec)
{
...
static void
anjuta_document_loader_finalize (GObject *object)
{
	AnjutaDocumentLoaderPrivate *priv = ANJUTA_DOCUMENT_LOADER (object)->priv;
	...
	G_OBJECT_CLASS (anjuta_document_loader_parent_class)->finalize (object);
}

static void
anjuta_document_loader_class_init (AnjutaDocumentLoaderClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = anjuta_document_loader_finalize;
	object_class->get_property = anjuta_document_loader_get_property;
	object_class->set_property = anjuta_document_loader_set_property;

	g_object_class_install_property (object_class,
					 PROP_DOCUMENT,
					 g_param_spec_object ("document",
							 "Document",
							 "The AnjutaDocument this AnjutaDocumentLoader is associated with",
	...
	signals[LOADING] =
   		g_signal_new ("loading",
			      ...
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (AnjutaDocumentLoaderClass, loading),
			      NULL, NULL,
	...
	g_type_class_add_private (object_class, sizeof(AnjutaDocumentLoaderPrivate));
}

static void
anjuta_document_loader_init (AnjutaDocumentLoader *loader)
{
	loader->priv = ANJUTA_DOCUMENT_LOADER_GET_PRIVATE (loader);
...
AnjutaDocumentLoader *
anjuta_document_loader_new (AnjutaDocument *doc)
{
	AnjutaDocumentLoader *dl;

	g_return_val_if_fail (ANJUTA_IS_DOCUMENT (doc), NULL);

	dl = ANJUTA_DOCUMENT_LOADER (g_object_new (ANJUTA_TYPE_DOCUMENT_LOADER, 
						  "document", doc,
						  NULL));
...
static void
insert_text_in_document (AnjutaDocumentLoader *loader,
			 const gchar         *text,
			 gint                 len)
{
...
static gboolean
update_document_contents (AnjutaDocumentLoader  *loader, 
			  ...
			  GError              **error)
{
	...
	if (loader->priv->encoding == anjuta_encoding_get_utf8 ())
	{
...
static void
load_completed_or_failed (AnjutaDocumentLoader *loader)
{
...
static gboolean
load_local_file_real (AnjutaDocumentLoader *loader)
{
	struct stat statbuf;
	GnomeVFSResult result;
	gint ret;
	...
	if (loader->priv->info->size == 0)
	{
		if (loader->priv->encoding == NULL)
			loader->priv->auto_detected_encoding = anjuta_encoding_get_current ();
...
static gboolean
open_local_failed (AnjutaDocumentLoader *loader)
{
	load_completed_or_failed (loader);
...
static void
load_local_file (AnjutaDocumentLoader *loader,
		 const gchar         *fname)
{
	g_signal_emit (loader,
		       signals[LOADING],
...
static void
remote_load_completed_or_failed (AnjutaDocumentLoader *loader)
{
...
static void	read_file_chunk		(AnjutaDocumentLoader *loader);
...
async_read_cb (GnomeVFSAsyncHandle *handle,
	       ...
	       gpointer             data)
{
	AnjutaDocumentLoader *loader = ANJUTA_DOCUMENT_LOADER (data);
...
static void
read_file_chunk (AnjutaDocumentLoader *loader)
{
...
static void
load_remote_file (AnjutaDocumentLoader *loader)
{
...
static gboolean
vfs_uri_new_failed (AnjutaDocumentLoader *loader)
{
	load_completed_or_failed (loader);
...
void
anjuta_document_loader_load (AnjutaDocumentLoader *loader,
			    const gchar         *uri,
			    const AnjutaEncoding *encoding)
{
	gchar *local_path;

	g_return_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader));
...
const gchar *
anjuta_document_loader_get_uri (AnjutaDocumentLoader *loader)
{
	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), NULL);

	return loader->priv->uri;
}

/* it may return NULL, it's up to anjuta-document handle it */
const gchar *
anjuta_document_loader_get_mime_type (AnjutaDocumentLoader *loader)
{
	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), NULL);
...
time_t
anjuta_document_loader_get_mtime (AnjutaDocumentLoader *loader)
{
	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), 0);
...
GnomeVFSFileSize
anjuta_document_loader_get_file_size (AnjutaDocumentLoader *loader)
{
	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), 0);
...
GnomeVFSFileSize
anjuta_document_loader_get_bytes_read (AnjutaDocumentLoader *loader)
{
	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), 0);
...
gboolean 
anjuta_document_loader_cancel (AnjutaDocumentLoader *loader)
{

	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), FALSE);
...
gboolean
anjuta_document_loader_get_readonly (AnjutaDocumentLoader *loader)
{
	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), FALSE);
...
anjuta_document_loader_get_encoding (AnjutaDocumentLoader *loader)
{
	g_return_val_if_fail (ANJUTA_IS_DOCUMENT_LOADER (loader), NULL);
