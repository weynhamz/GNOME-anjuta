 ...
 */
/****************************************************************/
/* This file is modified for the use with gtodo. 		*/
/* The following changes are made:				*/
/* - Fixed crashing when trying to set 
   a gdate withouth a valid dmy (but julian) 			*/
/* - Made the entry box one char widther. this to fix that the text doesnt fit */
/* - Added a today and no_date button to the date popup		*/
/*   And the needed interface for this.				*/
/* - The Date in the calendar is set tot the date represented by the datetime widget */
/****************************************************************/
...
static void cal_set_today (EggDateTime *edt, GtkCalendar *calendar);
static void cal_set_nodate (EggDateTime *edt, GtkCalendar *calendar);
...
/*static void
  ...
  */
/* This leaks..  and not little, so I remove this for now. */
...
static GtkWidget *timelist_new  (EggDateTime *edt);
static void timelist_set_list (Timelist *timelist, guint8 minhour, guint8 minminute, guint8 maxhour, guint8 maxminute);
static void timelist_set_time (Timelist *timelist, gint hour, gint minute);
static gboolean timelist_get_time (Timelist *timelist, gint *hour, gint *minute);
...
egg_datetime_init (EggDateTime *edt)
{
	...
	GtkWidget *arrow, *hbox, *vbox, *frame;
	...
	priv->week_start_monday = strcmp (_("week-starts-monday: yes"), "week-starts-monday: yes") == 0 ? TRUE : FALSE;
	...
	gtk_entry_set_width_chars (GTK_ENTRY (priv->date_entry), 13);
	/*
...
*/
	...
	gtk_window_set_focus(GTK_WINDOW(priv->cal_popup), priv->calendar);
	...
	priv->today = gtk_button_new_with_mnemonic(_("_Today"));
	priv->nodate = gtk_button_new_with_mnemonic(_("_No Date"));
	gtk_box_pack_start(GTK_BOX(hbox), priv->today, TRUE, TRUE,0);
	gtk_box_pack_start(GTK_BOX(hbox), priv->nodate, TRUE, TRUE,0);	
	g_signal_connect_swapped (G_OBJECT (priv->today), "clicked",
			G_CALLBACK (cal_set_today), edt);
	g_signal_connect_swapped (G_OBJECT (priv->nodate), "clicked",
			G_CALLBACK (cal_set_nodate), edt);
	...
	priv->timelist = timelist_new (edt);
	/*
		gtk_window_set_default(GTK_WINDOW(priv->time_popup), gtk_bin_get_child(GTK_BIN(priv->timelist)));
	*/
	...
	if(priv->no_date) gtk_widget_set_sensitive(priv->time_box, FALSE);
}
...
popup_show (GtkWindow *popup)
{
	...
	/* Think its ugly to change the cursor.. */
	/*
...
cal_popup_changed (EggDateTime *edt, GtkCalendar *calendar)
{
	...
	static void
cal_set_today (EggDateTime *edt, GtkCalendar *calendar)
{
	GDate *now;
	guint year, month, day;
	edt->priv->no_date = FALSE;
	...
	gtk_calendar_select_month(GTK_CALENDAR (edt->priv->calendar), month-1, year);
	gtk_calendar_select_day(GTK_CALENDAR (edt->priv->calendar), day);
	gtk_widget_show_all(edt->priv->calendar);
/*
		gtk_calendar_set_date (GTK_CALENDAR (edt->priv->calendar), &year, &month, &day);
*/
	normalize_date (edt);
	update_date_label (edt);
	...
	static void
cal_set_nodate (EggDateTime *edt, GtkCalendar *calendar)
{
	edt->priv->no_date = TRUE;   
...
void egg_set_nodate(EggDateTime *edt, gboolean val)    
{
	edt->priv->no_date = val;
...
gboolean egg_get_nodate(EggDateTime *edt)    
{
	return edt->priv->no_date;
}
...
cal_popup_double_click (EggDateTime *edt, GtkCalendar *calendar)
{
	edt->priv->no_date = FALSE;
	/*
		gtk_widget_set_sensitive(priv->time_box, edt->priv->no_date);
	*/
	cal_popup_hide (edt);
}
...
time_arrow_toggled (EggDateTime *edt, GtkToggleButton *button)
{
	...
	/* Show Popup */
	/* dirty hack to stop it selecting the first item when showing for the first time in new window*/
	/* I should fix this.. because this is quiet nasty */
	hour = priv->hour;
	minute= priv->minute;
...
update_date_label (EggDateTime *edt)
{
	EggDateTimePrivate *priv = edt->priv;
	gchar *s;
	gtk_widget_set_sensitive(priv->time_box, !priv->no_date);
	if(priv->no_date)
	{
		gtk_entry_set_text(GTK_ENTRY(priv->date_entry), _("No Date"));
		return;
	}
...
update_time_label (EggDateTime *edt)
{
	...
	if ((priv->display_mode & EGG_DATETIME_DISPLAY_SECOND) ||
		...
		if(priv->hour == -1 && priv->minute == 0)
			s = g_strdup(_("no end time"));
		else s = get_time_string (priv->hour, priv->minute, 0xff);
	}
...
parse_date (EggDateTime *edt)
{
	...
	if (!g_date_valid (&date)) {
		edt->priv->no_date = TRUE;
		gtk_entry_set_text(GTK_ENTRY(edt->priv->date_entry), _("No Date"));
		return;
	}
...
egg_datetime_set_from_gdate (EggDateTime *edt, GDate *date)
{
	...
	g_return_if_fail(g_date_valid_dmy(day, month, year));
 ...
 **/
	void
egg_datetime_set_time (EggDateTime *edt, gint hour, gint minute, guint8 second)
{
	g_return_if_fail (edt != NULL);
	g_return_if_fail (EGG_IS_DATETIME (edt));
	...
	update_time_label (edt);
	/*
		timelist_set_time(edt->priv->timelist, hour, minute);	
*/
	g_signal_emit (G_OBJECT (edt), egg_datetime_signals[SIGNAL_TIME_CHANGED], 0);
}
 ...
 **/
	gboolean
egg_datetime_get_time (EggDateTime *edt, gint *hour, gint *minute, gint *second)
{
	g_return_val_if_fail (edt != NULL, FALSE);
	g_return_val_if_fail (EGG_IS_DATETIME (edt), FALSE);
...
timelist_new  (EggDateTime *edt)
{
	...
	g_signal_connect_swapped(G_OBJECT(list), "row-activated", G_CALLBACK(time_popup_hide), edt);
		...
		guint8 maxhour, guint8 maxminute)
{
	...
	static void
timelist_set_time (Timelist *timelist, gint hour, gint minute)
{
	GtkWidget *tree = gtk_bin_get_child (GTK_BIN (timelist));
	GtkTreeModel *model;
	GtkTreeSelection *selection;
	...
	static gboolean
timelist_get_time (Timelist *timelist, gint *hour, gint *minute)
{
	GtkWidget *tree = gtk_bin_get_child (GTK_BIN (timelist));
	GtkTreeSelection *selection;
	GtkTreeModel *model;
