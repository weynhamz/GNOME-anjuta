...
get_property (GObject     *object,
			  ...
			  GParamSpec  *pspec)
{
  EggAccelDialog *dialog;
  
  dialog = EGG_ACCEL_DIALOG (object);
  
  switch (prop_id)
    {
    case PROP_MENU_MERGE:
      g_value_set_boxed (value, dialog->merge);
			  ...
			  GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MENU_MERGE:
	  g_message ("Setting merge property");
      dialog->merge = g_value_get_boxed (value);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
egg_accel_dialog_class_init (EggAccelDialogClass *class)
{
  ...
  g_object_class_install_property (gobject_class,
                                   PROP_MENU_MERGE,
                                   g_param_spec_string ("menu-merge",
                                                        _("Menu Merge"),
                                                        _("Menu Merge"),
                                                        NULL,
                                                        G_PARAM_READWRITE));
...
static gchar* accel_path_to_display_path (EggMenuMerge *merge,
...
egg_accel_dialog_init (EggAccelDialog *self)
{
  ...
  self->accel_store = gtk_list_store_new (N_COLUMNS, G_TYPE_STRING,
...
static gchar*
accel_path_to_display_path (EggMenuMerge *merge, const gchar *accel_path)
{
	gchar action[1024], *ret_str, *str;
	GString *display_path;
	if (!merge)
		return g_strdup (accel_path);

	display_path = g_string_new ("");
	const gchar *start, *end, *real_end;
	end = accel_path;
	real_end = accel_path + strlen (accel_path);
	while (end <= real_end)
	{
		start = end;
		while (*end != '/' && end <= real_end)
			end++;
		if (start[0] == '/')
			start++;
		if (end > start)
		{
			GList *action_groups, *ag_node;
			gboolean found = FALSE;
			
			display_path = g_string_append (display_path, "/");
			strncpy (action, start, end - start);
			action[end-start] = '\0';
			action_groups = merge->action_groups;
			ag_node = action_groups;
			while (ag_node)
			{
				EggAction *egg_action;
				EggActionGroup *group = EGG_ACTION_GROUP (ag_node->data);
				egg_action = egg_action_group_get_action (group, action);
				if (egg_action && egg_action->label &&
					strlen (egg_action->label))
				{
					found = TRUE;
					gchar *s, *d;
					gchar *str = g_strdup (egg_action->label);
					s = d = str;
					while (*s)
					{
					...
					*d = '\0';
					g_string_append (display_path, str);
					g_message ("Found label: %s", str);
					g_free (str);
				}
				if (found)
					break;
				ag_node = g_list_next (ag_node);
			}
			if (!found)
				display_path = g_string_append (display_path, action);
		}
		end++;
	}
	str = g_string_free (display_path, FALSE);
	if (strlen (str) > strlen ("/<actions>/") &&
		strncasecmp (str, "/<actions>/", strlen ("/<actions>/")) == 0)
	{
		ret_str = g_strdup (str + strlen ("/<actions>/"));
		g_free (str);
	}
	else
	{
		   ...
		   gboolean     changed)
{
  ...
  if (accel_key != 0)
    ...
    accel_name = "";
  display_path = accel_path_to_display_path (self->merge, accel_path);
...
GtkWidget *
egg_accel_dialog_new (EggMenuMerge *merge)
{
  GtkWidget *widget;
  widget = g_object_new(EGG_TYPE_ACCEL_DIALOG, NULL);
  EGG_ACCEL_DIALOG (widget)->merge = merge;
  egg_accel_dialog_rescan_accels (EGG_ACCEL_DIALOG (widget));
  return widget;
}
