...
static void
entry_activate (EggAction *action)
{
  g_message ("Entry activated");
}

static void
entry_changed (EggAction *action)
{
  g_message ("Entry changed");
}

static void
entry_focus_in (EggAction *action)
{
  g_message ("Entry focus in");
}

static void
entry_focus_out (EggAction *action)
{
  g_message ("Entry focus out");
}

static void
combo_activate (EggAction *action)
{
  g_message ("combo activated");
}

static void
combo_changed (EggAction *action)
{
  g_message ("combo changed");
}

static void
combo_focus_in (EggAction *action)
{
  g_message ("combo focus in");
}

static void
combo_focus_out (EggAction *action)
{
  g_message ("combo focus out");
}
...
main(int argc, char **argv)
{
  ...
  action = g_object_new (EGG_TYPE_ENTRY_ACTION,
			 "name", "ActionEntry",
			 "label", _("line entry"),
			 "tooltip", _("Enter the line number"),
			 "stock_id", GTK_STOCK_JUMP_TO,
			 "width", 50,
			 NULL);
  g_signal_connect (action, "activate", G_CALLBACK (entry_activate), NULL);
  g_signal_connect (action, "changed", G_CALLBACK (entry_changed), NULL);
  g_signal_connect (action, "focus-in", G_CALLBACK (entry_focus_in), NULL);
  g_signal_connect (action, "focus-out", G_CALLBACK (entry_focus_out), NULL);
  egg_action_group_add_action (action_group, action);

  action = g_object_new (EGG_TYPE_COMBO_ACTION,
			 "name", "ActionCombo",
			 "label", _("combo"),
			 "tooltip", _("line number"),
			 "stock_id", GTK_STOCK_JUMP_TO,
			 "width", 50,
			 NULL);
  g_signal_connect (action, "activate", G_CALLBACK (combo_activate), NULL);
  g_signal_connect (action, "changed", G_CALLBACK (combo_changed), NULL);
  g_signal_connect (action, "focus-in", G_CALLBACK (combo_focus_in), NULL);
  g_signal_connect (action, "focus-out", G_CALLBACK (combo_focus_out), NULL);
  egg_action_group_add_action (action_group, action);
