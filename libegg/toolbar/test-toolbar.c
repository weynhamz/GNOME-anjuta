	       ...
	       GtkTreeModel *model)
{
  ...
  gtk_tree_model_get (model, &iter, 0, &tool_item, -1);
  egg_tool_item_set_expand (tool_item, !tool_item->expand);
  g_object_unref (tool_item);
...
main (gint argc, gchar **argv)
{
  ...
  egg_tool_button_set_label (EGG_TOOL_BUTTON (item), "Custom label");
  egg_tool_button_set_label (EGG_TOOL_BUTTON (item), NULL);
  ...
  item = egg_radio_tool_button_new_from_stock (group, GTK_STOCK_JUSTIFY_CENTER);  make_prop_editor (G_OBJECT (item));
