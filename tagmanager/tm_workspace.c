...
tm_create_workspace (void)
{
	...
	workspace_class_id = tm_work_object_register (tm_workspace_free,
												  tm_workspace_update,
...
void
tm_workspace_free (gpointer workspace)
{
	guint i;
...
gboolean
tm_workspace_add_object (TMWorkObject * work_object)
{
	if (NULL == theWorkspace)
		tm_create_workspace ();
...
gboolean
tm_workspace_remove_object (TMWorkObject * w, gboolean free)
{
	guint i;
	...
	for (i = 0; i < theWorkspace->work_objects->len; ++i)
	{
		if (theWorkspace->work_objects->pdata[i] == w)
		{
			...
			tm_workspace_update (TM_WORK_OBJECT (theWorkspace), TRUE,
								 FALSE, FALSE);
			return TRUE;
		}
	}
...
static GPtrArray *
tm_workspace_load_tags (GPtrArray * array, const char *tags_file)
{
	FILE *fp;
	TMTag *tag;
	GPtrArray *tags;
...
static void
tm_move_entries_to_g_list (gpointer key, gpointer value, gpointer user_data)
{
	if (user_data == NULL)
		return;
...
gboolean
tm_workspace_create_global_tags (const char *pre_process, const char **includes,
								 int includes_count, const char *tags_file)
{
	...
	command =
		g_strdup_printf
		("%s %s | grep -v -E '^\\s*(G_BEGIN_DECLS|G_END_DECLS)\\s*$' > %s",
...
gboolean
tm_workspace_merge_global_tags (const gchar * output_file, GList * tag_files)
{
	int i;
	FILE *fp;
	GList *node;
	...
	while (node)
	{
		...
		if (str_has_suffix ((const char *) node->data, ".gz"))
		{
			...
			command = g_strdup_printf ("gunzip -c '%s' > %s",
									   (const char *) node->data, temp_file);
			system (command);
			g_free (command);
...
TMWorkObject *
tm_workspace_find_object (TMWorkObject * work_object, const char *file_name,
						  gboolean name_only)
{
	TMWorkObject *w = NULL;
	guint i;
	...
	for (i = 0; i < theWorkspace->work_objects->len; ++i)
	{
		if (NULL !=
			(w =
			 tm_work_object_find (TM_WORK_OBJECT
								  (theWorkspace->work_objects->pdata[i]),
								  file_name, name_only)))
			return w;
	}
	return NULL;
...
void
tm_workspace_recreate_tags_array (void)
{
	guint i, j;
	TMWorkObject *w;
	TMTagAttrType sort_attrs[] =
		{ tm_tag_attr_name_t, tm_tag_attr_file_t, tm_tag_attr_scope_t,
tm_tag_attr_type_t, 0
	};
...
gboolean
tm_workspace_update (TMWorkObject * workspace, gboolean force,
...
tm_workspace_dump (void)
{
	if (theWorkspace)
	{
		...
		if (theWorkspace->work_objects)
		{
			...
			{
				if (IS_TM_PROJECT
					(TM_WORK_OBJECT (theWorkspace->work_objects->pdata[i])))
					tm_project_dump (TM_PROJECT
									 (theWorkspace->work_objects->pdata[i]));
				else
					tm_work_object_dump (TM_WORK_OBJECT
										 (theWorkspace->work_objects->
										  pdata[i]));
			}
		}
	}
					  ...
					  gboolean first)
{
	...
	if (name == NULL)
	{
		/* Simply copy all src tags to dst tags */
		int i;
		for (i = 0; i < src->len; i++)
		{
			if (TM_TAG (src->pdata[i])->type & type)
			{
				g_ptr_array_add (dst, src->pdata[i]);
			}
		}
		return dst->len;
	}
...
const GPtrArray *
tm_workspace_find (const char *name, int type, TMTagAttrType * attrs,
			...
			while (scope)
			{
				...
				if (scope)
				{
					...
					for (j = 0; (j < local->len); ++j)
					{
						...
						if (tag2->atts.entry.type_ref[1] &&
							0 == strcmp (var_type, tag2->atts.entry.type_ref[1]))
						{
							break;
						}
					}
								 ...
								 gboolean search_global)
{
	...
	while (1)
	{
		...
		else
		{
			...
			tags2 = tm_workspace_find (new_name, types, attrs, FALSE, TRUE);
		}
...
const GPtrArray *
tm_workspace_get_parents (const gchar * name)
{
	...
	matches = tm_workspace_find (name, tm_tag_class_t, type, FALSE, TRUE);
	...
	while (i < parents->len)
	{
		...
		if ((NULL != tag->atts.entry.inheritance) &&
			(isalpha (tag->atts.entry.inheritance[0])))
		{
			...
			for (klass = klasses; (NULL != *klass); ++klass)
			{
				...
				if (parents->len == j)
				{
					matches = tm_workspace_find (*klass, tm_tag_class_t,
												 type, FALSE, TRUE);
					if ((NULL != matches) && (0 < matches->len))
						g_ptr_array_add (parents, matches->pdata[0]);
				}
