...
gchar*
gtk_vim_get_server_name (GtkVim *widget)
{
    GtkVim *vim;
              ...
              ...)
{
	...
	*/
	...
	g_message ("Adding files to edit");
    va_start( args, filename );

	/* If the server is not yet started, put the files in init files list */
	if (vim->server_name)
	{
	...
	{
		tmp = vim->init_files;
		vim->init_files = g_strconcat(tmp, " ", filename, NULL );
		g_free( tmp );
	}
    for (arg = (gchar *)va_arg( args, gchar * );  arg; )
    {
...
void
gtk_vim_remote_send (GtkVim *widget,
                     const gchar     *keys)
{
    GtkVim *vim;
    gchar  *cmd;
