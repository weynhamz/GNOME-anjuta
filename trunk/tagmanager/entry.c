...
static int writeCtagsEntry (const tagEntryInfo *const tag)
{
	...
	if (tag == NULL || tag->name == NULL ||
	    tag->sourceFileName == NULL) {
		printf("**warning**: returning -1 from writeCtagsEntry()\n");
	    return -1;
	}
...
extern void makeTagEntry (const tagEntryInfo *const tag)
{
	...
	else
	{
		...
		else if (Option.xref)
		{
			...
			// it should crash here. Infact no TagFile.fp is initialized right now.
			// We're using ctags within tagmanager.
